export default class Maze2 {
    ...
    public init(size: number): void {
        if (!this.running) {
            ...
            for (let i = 0; i < this.imgData.data.length; i++) {
                remainder = i % 4
                if (remainder == Maze2.ALPHA) imgArr[i] = 0xff
                else imgArr[i] = this.bgColor[remainder]!
            }
            if (this.dx > 2)
    ...
    public setMap(coor: Coordinate, value: number): void {
        if (value > this.maxWalk) this.maxWalk = value
                ...
                this.teams.forEach((runner) => {
                    if (runner.isActive()) {
                        ...
                        if (runner.getDirection() == Maze2.NONE) {
                            runner.routeBack()
                            if (Math.random() < connect) runner.breakTheWall()
                            runner.move()
...
class Runner {
    ...
    public move(): void {
        ...
        if (this.direction == Maze2.NONE) {
            ...
            if (this.direction == Maze2.NORTH) this.locate.i--
            else if (this.direction == Maze2.WEST) this.locate.j--
            else if (this.direction == Maze2.SOUTH) this.locate.i++
            else if (this.direction == Maze2.EAST) this.locate.j++
            if (this.isBackward()) {
    ...
    public routeBack(): void {
        ...
        if (this.route.length > 0) {
            ...
            if (prior.i < this.locate.i) this.direction = Maze2.NORTH
            else if (prior.j < this.locate.j) this.direction = Maze2.WEST
            else if (prior.i > this.locate.i) this.direction = Maze2.SOUTH
            else if (prior.j > this.locate.j) this.direction = Maze2.EAST
            this.backward = true
        }
    }
    ...
    public findNewPath(): void {
        ...
        if (
            ...
            if (idx == 1) this.direction = Maze2.WEST
            direct += Maze2.WEST
        }
        if (
            this.direction == Maze2.NONE &&
        ...
        if (direct > Maze2.NONE) {
            this.backward = false
            if (this.direction == Maze2.NONE) {
                if (idx == 0) this.direction = Maze2.EAST
                else if (idx == 1) this.direction = Maze2.NORTH
