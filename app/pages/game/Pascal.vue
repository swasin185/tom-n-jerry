...
class Ball extends Sprite {
    ...
    move(): void {
        ...
        if (this.speed < 0.001) this.gravity = GRAVITY
    ...
    setDirection(d: number): void {
        ...
        if (this.dy > -1e-4 && this.dy < 1e-4) this.dy = 0
        if (this.dx > -1e-4 && this.dx < 1e-4) this.dx = 0
    }
    ...
    bounce(x1: number, y1: number, x2: number, y2: number): boolean {
        ...
        if (this.x < x1 + this.radius || this.x > x2 - this.radius) {
            if (this.x < x1 + this.radius) this.x = x1 + this.radius
            else this.x = x2 - this.radius
            this.reflect(0)
            isBounce = true
        }
        if (this.y < y1 + this.radius || this.y > y2 - this.radius) {
            if (this.y < y1 + this.radius) this.y = y1 + this.radius
            else {
...
async function calculate() {
    ...
    while (true) {
        nearBall = false
        for (let i = 0; i < balls.length && !nearBall; i++) nearBall = balls[i]!.getY() < nearArea
        if (!nearBall) {
            resetBall = false
        ...
        for (let ball of balls) {
            ...
            if (!ball.isRemoved()) {
                ...
                ball.move()
                if (ball.getY() > floor) ball.remove()
            }
        }
        paint()
...
function paint() {
    ...
    for (let box of boxs) box.draw()
    for (let ball of balls) if (!ball.isRemoved()) ball.draw()
}
